...
custom_db=../../../../compare_algorithms/prepare_data/run_CellPhoneDB/build_customDB/CPDB_Custom/CPDB_Custom.db
...
do
  ...
  cellphonedb method degs_analysis --threshold 0.1 --threads 1 ${samples_dir}${sample}_meta.tsv ${samples_dir}${sample}_counts.tsv ${samples_dir}${sample}_DEGs.tsv --database $custom_db --counts-data hgnc_symbol --output-path ${samples_dir}${sample}_results/;
done;
